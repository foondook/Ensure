...
");
        ...
        foreach (var step in uniqueSteps)
        {
            ...
            builder.AppendLine($@"
        public async Task {methodName}({parameterList})
        {{
            // TODO: Implement step logic
            throw new NotImplementedException($""Step not implemented: {step.Text}"");
        }}");
        }
...
{{
    ...
    {{
        private readonly {stepsClassName} _steps;

        public {className}Tests()
        {{
            _steps = new {stepsClassName}();
        }}
");
    ...
    private static string ToValidMethodName(string stepText)
    {
        ...
        var methodName = string.Join("", words.Select(w => char.ToUpper(w[0]) + w.Substring(1).ToLower()));
        
        // Ensure it ends with Async
...
");
        ...
        {
            builder.AppendLine($"            // {step.Text}");
            var methodName = ToValidMethodName(step.Text);
            var parameters = string.Join(", ", step.Parameters.Values.Select(v => $"\"{v}\""));
            builder.AppendLine($"            await _steps.{methodName}({parameters});");
        }
